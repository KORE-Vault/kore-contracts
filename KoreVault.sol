...
// Kore Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract KoreVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KOREs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accKorePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accKorePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. KOREs to distribute per block.
        uint256 accKorePerShare; // Accumulated KOREs per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing KORE governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's KOREs allocation point. Can only be called by the owner.
    // Note contract owner is meant to be a governance contract allowing KORE governance consensus
    ...
    // View function to see pending KOREs on frontend.
    function pendingKore(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accKorePerShare = pool.accKorePerShare;

        return user.amount.mul(accKorePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the KORE token.
    // ----
    uint256 private koreBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = kore.balanceOf(address(this)).sub(koreBalance);

        if(newRewards > 0) {
            koreBalance = kore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 koreRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        koreRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 koreRewardFee = koreRewardWhole.mul(DEV_FEE).div(10000);
        uint256 koreRewardToDistribute = koreRewardWhole.sub(koreRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(koreRewardFee);

        pool.accKorePerShare = pool.accKorePerShare.add(
            koreRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to KoreVault for KORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accKorePerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accKorePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from KoreVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming KORE farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accKorePerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeKoreTransfer(from, pending);
        }
    ...
    // Safe kore transfer function, just in case if rounding error causes pool to not have enough KOREs.
    function safeKoreTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 koreBal = kore.balanceOf(address(this));
        if (_amount > koreBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", koreBal);

            kore.transfer(_to, koreBal);
            koreBalance = kore.balanceOf(address(this));

        } else {
            kore.transfer(_to, _amount);
            koreBalance = kore.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeKoreTransfer(devaddr, devSend);
        }
    ...
    // allowing KORE governance token holders to do this functions.
